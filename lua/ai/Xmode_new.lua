...
function getRandomGenerals(n,kingdom,exceptions)
	...
	if kingdom == "careerist" then
		local count = 0
		local c_num = math.min(5, n)
		for _,general in pairs(hash["careerist"])do
			if #result == c_num then break end
			if not table.contains(exceptions,general) then
				table.insert(result,general)
				count = count + 1
			end
		end
		if #result < c_num then
			local kingdoms = {"wei","shu","wu","qun"}
			kingdoms = table.Shuffle(kingdoms)
			for i = 1, c_num - #result, 1 do
				local role = kingdoms[i]
				hash[role] = table.Shuffle(hash[role])
				if not table.contains(exceptions,hash[role][1]) then
					table.insert(result,hash[role][1])
					count = count + 1
				end
			end
		end
...
local random_count = {["wei"] = 0,["shu"] = 0,["wu"] = 0,["qun"] = 0}
local max_num = 5
local num_count = 0
for key, _ in pairs(random_count) do
	if max_num > 0 then
		local num = math.random(1,max_num)
		random_count[key] = num
		num_count = num_count + num
		max_num = math.min(5, 10 - num_count)
	end
end
...
XmodeRule = sgs.CreateTriggerSkill{
	...
	on_effect = function(self, evnet, room, player, data,ask_who)
		...
		local from,revive_k
		if damage and damage.from and damage.from:getRole() ~= "careerist" and damage.from:getKingdom() ~= "careerist"
		and player:getRole() ~= "careerist" then
			from = damage.from
			revive_k = damage.from:getKingdom()
		...
		if times + random_count[change_k] >= 5 then
			return false
		end
		local used = room:getTag("Xmode_UsedGeneral"):toString():split("+")
		local random_general = getRandomGenerals(sgs.GetConfig("HegemonyMaxChoice",0),player:getActualGeneral1():getKingdom(),used)
...
Xmode = {
	...
	on_assign = function(self, room)
		...
		for i = 1, 10 - #kingdoms, 1 do
			local role = kingdom[math.random(1,#kingdom)]
			if rules_count[role] < random_count[role] then
				rules_count[role] = rules_count[role] + 1
				table.insert(kingdoms, role)
			end
			if rules_count[role] == random_count[role] then
				table.removeOne(kingdom,role)
			end
		end
		--上面的代码是为了随机分配国家，随机移除一个势力，每个势力有随机人数
